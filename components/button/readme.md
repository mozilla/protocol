The `mzp-c-button` class can be a applied to a `button` element, an `input type="button"` or an `a` element. Button size and color themes can be combined as needed.
